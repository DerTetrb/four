...
public class Vector3DArray {
    ...
    public int contains(Vector3D v) {
        ...
        for (int i = 0; i < this.arr.length; i++) {
            if (arr[i].equalsVector(v)) return i;
        }
        return -1;
    }
    ...
    public Vector3D sumAllVectors() {
        ...
        for (int i = 1; i < this.arr.length; i++) {
            res = Vector3DProcessor.sumVector3D(res, arr[i]);
        }
        return res;
    }

    public Vector3D lineCombination(double[] kof) {
        if (kof.length != arr.length) throw new IllegalArgumentException();
        double x = 0;
        double y = 0;
        double z = 0;
